...
int current_floor = 0;  // current floor at the start of the program is unknown need calibration
...
void func4(){
  //Serial.println("\t\tfloor 1 button in the lift is pressed");
  if (current_floor == 1 && in_motion == 0) {
    is_open=1;
  }else {  
    down_calls[1] = 1;
...
void service(){
  ...
  if(up_calls[0]==1){
    ...
    //update current floor value based on value from potentiometer when calibrating
      if(current_floor==2){
          if(pot  == floors[2]-15){
            Serial.println("incrementing floor to 3");
            current_floor++;
          ...
          if(pot  ==floors[1]-15){
              Serial.println("incrementing floor to 2");
              current_floor++;
          }
  ...
  // check for down routine
    ...
    if(current_floor == 3){
        if(pot ==floors[1]+15){
          Serial.println("decrementing floor to 2");
          current_floor--;
        }
        
     }else if(current_floor == 2){
         if(pot==floors[0]+15){
            Serial.println("decrementing floor to 1");
            current_floor--;
          }
